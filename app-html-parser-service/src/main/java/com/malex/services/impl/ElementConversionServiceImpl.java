...
@Service
public class ElementConversionServiceImpl extends AbstractService implements ElementConversionService {

    @Override
    public Flux<BillStatus> convertElementsToBillStatuses(List<Elements> elements) {
        List<BillStatus> statuses = elements.stream() //
                .map(this::buildBillStatusObj) //
    ...
    public Flux<Bill> convertElementsToBills(List<Elements> elements) {
        List<Bill> bills = elements.stream() //
                .map(this::buildBillObj) //
                .collect(Collectors.toList());
