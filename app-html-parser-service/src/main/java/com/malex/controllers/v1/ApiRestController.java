...
public class ApiRestController {
  ...
  public Mono<SearchResponse> findBillByNumber(
    ...
    FormData formData =
        FormData.builder()
            ...
            .build();
    return restService
        .fetchSearchResult(formData.getFormData())
        ...
        .doOnNext(
            message ->
                log.info("Search API response processing completed, response - {}", message));
  }
  ...
  public Mono<BillResponse> findBillStatuses(@RequestBody BillRequest request) {
    log.info(
        "Start processing find bill statuses to search statuses and parse HTML page, request - {}",
        request);
    return restService
        ...
        .doOnNext(
            message -> log.info("Parse API response processing completed, response - {}", message));
  }
